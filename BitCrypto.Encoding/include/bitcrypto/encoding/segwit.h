...
inline std::string p2wpkh_from_priv(const uint8_t priv32_be[32], bool compressed, bool testnet){
    ...
    std::vector<uint8_t> prog(h160, h160+20); std::vector<uint8_t> prog5;
    convert_bits(prog, 8, 5, true, prog5);
    std::vector<uint8_t> data; data.push_back(0); data.insert(data.end(), prog5.begin(), prog5.end());
    std::string out; bech32_encode(segwit_hrp(testnet), data, Bech32Variant::BECH32, out);
    return out;
}
inline bool segwit_decode_address(const std::string& addr, std::string& hrp, int& version, std::vector<uint8_t>& program){
    ...
    if(!bech32_decode(addr, hrp, data, var)) return false;
    if (data.size()<1) return false;
    version = (int)data[0];
    if (version<0 || version>16) return false;
    if ((version==0 && var==Bech32Variant::BECH32M) || (version>=1 && var==Bech32Variant::BECH32)) return false;
    std::vector<uint8_t> data_nover(data.begin()+1, data.end());
    program.clear();
    if(!convert_bits(data_nover, 5, 8, false, program)) return false;
