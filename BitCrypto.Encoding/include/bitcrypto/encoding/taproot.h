...
inline std::string p2tr_from_priv(const uint8_t priv32_be[32], bool testnet){
    ...
    uint8_t qx[32]; bool _; normalize_even_y(Q, qx, _);
    std::vector<uint8_t> prog(qx, qx+32); std::vector<uint8_t> prog5;
    convert_bits(prog, 8, 5, true, prog5);
    std::vector<uint8_t> data; data.push_back(1); data.insert(data.end(), prog5.begin(), prog5.end());
    std::string out; bech32_encode(segwit_hrp(testnet), data, Bech32Variant::BECH32M, out); return out;
}
