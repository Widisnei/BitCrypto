...
inline bool sign_input_p2tr_keypath(Transaction& tx, size_t in_idx,
    ...
    uint8_t m[32]; taproot_sighash_keypath(tx, in_idx, amounts, scriptPubKeys, sh, m);
    ...
    tx.vin[in_idx].witness.clear(); tx.vin[in_idx].witness.push_back(w0); tx.has_witness=true; tx.set_segwit_if_any_witness();
    return true;
}
