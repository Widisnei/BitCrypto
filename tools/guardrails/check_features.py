...
tokens = {
    ...
    "PSBTCLI": ["--psbt2-pretty", "--psbt2-decode", "--psbt2-verify", "witness_items=", "witness_sizes="],
...
for key, rel in anchors.items():
    p = ROOT/rel
    if not p.exists():
        print("[FALTA]", rel); ok=False; continue
    txt = p.read_text(encoding="utf-8", errors="ignore")
    for tok in tokens.get(key, []):
        if tok not in txt:
            print("[FALTA TOKEN]", rel, "=>", tok); ok=False
print("Features OK" if ok else "Features INCOMPLETO")
sys.exit(0 if ok else 1), "SchnorrCLI": ["--bip340-verify"], "PuzzleCLI": ["--kangaroo"],
 "WSCLI": ["witness_items=", "witness_sizes=", "tap_control_block_depth="],
 "PSBTv2": ["unknown_in_kv"],
 "EC": ["wnaf_w4", "precompute_G_w4", "scalar_mul_base_wnaf"],
